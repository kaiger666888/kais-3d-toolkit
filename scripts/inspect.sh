...
def inspect_model(filepath, output_dir):
    ...
    for mat in bpy.data.materials:
        ...
        # 检测材质模式
        # Blender 4.x EEVEE blend_method
...
inspect_model(sys.argv[-2], sys.argv[-1])
PYEOF
)

"$BLENDER" --background --python-expr "$PYTHON_SCRIPT" -- "$OUT_DIR" "$GLB_FILE" 2>&1 | grep -v "^Blender\|^Read\|^Fra:"
